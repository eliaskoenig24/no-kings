...
import { DAILY_MIN_PERSONS, groupByRegion } from '@/lib/network-policy';
  ...
  return (
    <div style={{ padding: 'clamp(48px, 7vw, 88px) 0' }}>
      <div className="container" style={{ maxWidth: '780px' }}>
        ...
        {/* The gate: analysis opens once your own twin exists. Reciprocity by
            design — you cannot read the world without positioning yourself.
            (Gated on a LOCAL twin, not on publishing: GDPR consent to publish
            special-category data must stay freely given, never a paywall.) */}
        {loaded && !myTwin && !simView && (
          <div style={{ padding: '24px 0 40px' }}>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.2em', color: 'var(--accent)', textTransform: 'uppercase', marginBottom: '18px' }}>
              {tx(lang, 'gate_label')}
            </p>
            <h1 style={{ fontSize: 'clamp(1.6rem, 4.5vw, 2.6rem)', fontWeight: 600, lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: '16px', maxWidth: '20ch' }}>
              {tx(lang, 'gate_title')}
            </h1>
            <p style={{ fontSize: '15px', lineHeight: 1.7, color: 'var(--text-2)', maxWidth: '560px', marginBottom: '28px' }}>
              {tx(lang, 'gate_body')}
            </p>
            <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <button onClick={() => setSimView(true)} style={{
                background: 'none', border: '1px solid var(--border)', color: 'var(--text-3)',
                padding: '14px 24px', fontSize: '12px', cursor: 'pointer',
        ...
        {/* Bottom CTA (simulation viewers without a twin) */}
        {loaded && !myTwin && (
