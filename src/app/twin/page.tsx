...
const TX = {
  ...
  consent_geo: { de: 'Mit veröffentlicht werden — falls angegeben — nur Land und grobe Region (z. B. Bayern), nie genauere Ortsdaten.', en: 'If provided, only your country and coarse region (e.g. Bavaria) are published — never anything more precise.', es: 'Si los indicas, solo se publican tu país y región aproximada (p. ej. Baviera) — nunca datos más precisos.', fr: 'Si tu les indiques, seuls ton pays et ta région approximative (p. ex. la Bavière) sont publiés — jamais plus précis.', pt: 'Se informados, apenas seu país e região aproximada (ex. Baviera) são publicados — nunca algo mais preciso.', ar: 'إذا حددتهما، يُنشر فقط بلدك ومنطقتك التقريبية — ولا شيء أدق أبدًا.', zh: '如果你填写，只会发布你的国家和粗略地区（如巴伐利亚）——绝不会发布更精确的位置。', ja: '入力した場合も、公開されるのは国とおおまかな地域（例：バイエルン州）のみ — それ以上詳しい位置は決して公開されません。', hi: 'यदि आप बताते हैं, तो केवल देश और मोटा क्षेत्र (जैसे बवेरिया) प्रकाशित होता है — इससे सटीक कुछ नहीं।', ru: 'Если вы их указали, публикуются только страна и грубый регион (напр. Бавария) — ничего точнее.', id: 'Jika diisi, hanya negara dan wilayah kasar (mis. Bavaria) yang dipublikasikan — tidak pernah lebih presisi.', tr: 'Belirtirsen yalnızca ülken ve kaba bölgen (örn. Bavyera) yayımlanır — asla daha kesin bir konum değil.', ko: '입력한 경우 국가와 대략적인 지역(예: 바이에른)만 공개되며, 더 정확한 위치는 절대 공개되지 않습니다.', it: 'Se indicati, vengono pubblicati solo il tuo paese e la regione approssimativa (es. Baviera) — mai nulla di più preciso.', nl: 'Indien opgegeven worden alleen je land en grove regio (bijv. Beieren) gepubliceerd — nooit iets preciezer.', pl: 'Jeśli je podasz, publikowane są tylko kraj i przybliżony region (np. Bawaria) — nigdy nic dokładniejszego.', uk: 'Якщо ви їх вказали, публікуються лише країна та приблизний регіон (напр. Баварія) — нічого точнішого.', vi: 'Nếu bạn cung cấp, chỉ quốc gia và vùng thô (vd. Bavaria) được công bố — không bao giờ chính xác hơn.', bn: 'দিলে শুধু দেশ ও স্থূল অঞ্চল (যেমন বাভারিয়া) প্রকাশিত হয় — এর চেয়ে নিখুঁত কিছু নয়।', fa: 'در صورت ارائه، فقط کشور و منطقه تقریبی شما (مثلاً بایرن) منتشر می‌شود — هرگز دقیق‌تر از آن.' },
